...
function memorizaNomesEmPartes() { localStorage.setItem('nomes6B', JSON.stringify(nomes6B)); }
if (!localStorage.getItem('nomes6B')) { memorizaNomesEmPartes(); }

function getLocalStorage() { return JSON.parse(localStorage.getItem('nomes6B')) ?? { nomet: [], nota1t: [], nota2t: [], nota3t: [] }; }
function setLocalStorage(nomes6B) { localStorage.setItem('nomes6B', JSON.stringify(nomes6B)); }

function ordenarNomesComNotas(nomes6B) {
    const combinados = nomes6B.nomet.map((nome, index) => ({ nome: nome, nota1: nomes6B.nota1t[index], nota2: nomes6B.nota2t[index], nota3: nomes6B.nota3t[index] }));
    combinados.sort((a, b) => a.nome.localeCompare(b.nome));
    nomes6B.nomet = combinados.map(item => item.nome);
    nomes6B.nota1t = combinados.map(item => item.nota1);
    nomes6B.nota2t = combinados.map(item => item.nota2);
    nomes6B.nota3t = combinados.map(item => item.nota3);
}
...
function atualizarTabela() {
    ...
    const nomes6B = getLocalStorage();
    nomes6B.nomet.forEach((nome, index) => { inserirLinhaTabela(nome, index, nomes6B.nota1t[index], nomes6B.nota2t[index], nomes6B.nota3t[index]); });
}
...
function inserirNota() {
    ...
    if (parseFloat(aluno.nota1) <= 10 && parseFloat(aluno.nota2) <= 10 && parseFloat(aluno.nota3) <= 10) {
        if (parseFloat(aluno.nota1) >= 0 && parseFloat(aluno.nota2) >= 0 && parseFloat(aluno.nota3) >= 0) {
            let nomes6B = getLocalStorage();
            if (editaId === null) {
                nomes6B.nomet.push(aluno.nome);
                nomes6B.nota1t.push(aluno.nota1);
                nomes6B.nota2t.push(aluno.nota2);
                nomes6B.nota3t.push(aluno.nota3);
                ordenarNomesComNotas(nomes6B);
                setLocalStorage(nomes6B);
                registrarNoDashboard('adicionado', aluno.nome, '6º Ano - B', 'Novo aluno matriculado');
                alert("Aluno adicionado com sucesso!");
                atualizarTabela();
                add.style.display = 'block';
                formulario.style.display = 'none';
            } else { editarNota(nomes6B, editaId, aluno); }
        } else { alert('Coloque uma nota entre 0 e 10'); }
    } else { alert('Coloque uma nota entre 0 e 10'); }
}

function editarNota(nomes6B, index, aluno) {
    nomes6B.nomet[index] = aluno.nome;
    nomes6B.nota1t[index] = aluno.nota1;
    nomes6B.nota2t[index] = aluno.nota2;
    nomes6B.nota3t[index] = aluno.nota3;
    ordenarNomesComNotas(nomes6B);
    setLocalStorage(nomes6B);
    botao.value = "Inserir";
    editaId = null;
...
function excluirNota(index) {
    let nomes6B = getLocalStorage();
    const nome = nomes6B.nomet[index];
    ...
    if (motivo) {
        ...
        if (confirmarExclusao) {
            ...
            if (senha === '12345') {
                ...
                nomes6B.nomet.splice(index, 1);
                nomes6B.nota1t.splice(index, 1);
                nomes6B.nota2t.splice(index, 1);
                nomes6B.nota3t.splice(index, 1);
                setLocalStorage(nomes6B);
                atualizarTabela();
                registrarNoDashboard('excluido', nome, '6º Ano - B', motivo);
...
function preparaEditacao(index) {
    let nomes6B = getLocalStorage();
    editaId = index;
    const nome1 = nomes6B.nomet[index];
    ...
    if (confirmar) {
        ...
        if (senhae === '12345') {
            ...
            nome.value = nome1;
            nota1.value = nomes6B.nota1t[index];
            nota2.value = nomes6B.nota2t[index];
            nota3.value = nomes6B.nota3t[index];
