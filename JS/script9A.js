...
const nomes9A = {
    nomet: [
        "AliceSilvaSantos", "BrunoAlmeidaCosta", "CarlaLimaOliveira",
        "DiegoPereiraRodrigues", "EvelynCostaAlmeida", "FelipeLimaSantos",
        "GiovannaOliveiraPereira", "HenriqueCostaLima", "IsadoraSantosAlmeida",
        "JoãoVitorPereiraCosta", "KatherineLimaOliveira", "LucasAlmeidaSantos",
        "MarianaCostaPereira", "NicolasLimaAlmeida", "OliviaSantosCosta",
        "PedroHenriquePereiraLima", "RafaelaCostaOliveira", "SamuelAlmeidaSantos",
        "TatianePereiraCosta", "VictorHugoLimaOliveira", "YasminAlmeidaPereira",
        "ZoeCostaSantos", "ArthurOliveiraLima", "BeatrizAlmeidaCosta",
        "CaioPereiraOliveira"
    ],
...
function memorizaNomesEmPartes() { localStorage.setItem('nomes9A', JSON.stringify(nomes9A)); }
if (!localStorage.getItem('nomes9A')) { memorizaNomesEmPartes(); }

function getLocalStorage() { return JSON.parse(localStorage.getItem('nomes9A')) ?? { nomet: [], nota1t: [], nota2t: [], nota3t: [] }; }
function setLocalStorage(nomes9A) { localStorage.setItem('nomes9A', JSON.stringify(nomes9A)); }

function ordenarNomesComNotas(nomes9A) {
    const combinados = nomes9A.nomet.map((nome, index) => ({ nome: nome, nota1: nomes9A.nota1t[index], nota2: nomes9A.nota2t[index], nota3: nomes9A.nota3t[index] }));
    combinados.sort((a, b) => a.nome.localeCompare(b.nome));
    nomes9A.nomet = combinados.map(item => item.nome);
    nomes9A.nota1t = combinados.map(item => item.nota1);
    nomes9A.nota2t = combinados.map(item => item.nota2);
    nomes9A.nota3t = combinados.map(item => item.nota3);
}
...
function atualizarTabela() {
    ...
    const nomes9A = getLocalStorage();
    nomes9A.nomet.forEach((nome, index) => { inserirLinhaTabela(nome, index, nomes9A.nota1t[index], nomes9A.nota2t[index], nomes9A.nota3t[index]); });
}
...
function inserirNota() {
    ...
    if (parseFloat(aluno.nota1) <= 10 && parseFloat(aluno.nota2) <= 10 && parseFloat(aluno.nota3) <= 10) {
        if (parseFloat(aluno.nota1) >= 0 && parseFloat(aluno.nota2) >= 0 && parseFloat(aluno.nota3) >= 0) {
            let nomes9A = getLocalStorage();
            if (editaId === null) {
                nomes9A.nomet.push(aluno.nome);
                nomes9A.nota1t.push(aluno.nota1);
                nomes9A.nota2t.push(aluno.nota2);
                nomes9A.nota3t.push(aluno.nota3);
                ordenarNomesComNotas(nomes9A);
                setLocalStorage(nomes9A);
                registrarNoDashboard('adicionado', aluno.nome, '9º Ano - A', 'Novo aluno matriculado');
                alert("Aluno adicionado com sucesso!");
                atualizarTabela();
                add.style.display = 'block';
                formulario.style.display = 'none';
            } else { editarNota(nomes9A, editaId, aluno); }
        } else { alert('Coloque uma nota entre 0 e 10'); }
    } else { alert('Coloque uma nota entre 0 e 10'); }
}

function editarNota(nomes9A, index, aluno) {
    nomes9A.nomet[index] = aluno.nome;
    nomes9A.nota1t[index] = aluno.nota1;
    nomes9A.nota2t[index] = aluno.nota2;
    nomes9A.nota3t[index] = aluno.nota3;
    ordenarNomesComNotas(nomes9A);
    setLocalStorage(nomes9A);
    botao.value = "Inserir";
    editaId = null;
...
function excluirNota(index) {
    let nomes9A = getLocalStorage();
    const nome = nomes9A.nomet[index];
    ...
    if (motivo) {
        ...
        if (confirmarExclusao) {
            ...
            if (senha === '12345') {
                ...
                nomes9A.nomet.splice(index, 1);
                nomes9A.nota1t.splice(index, 1);
                nomes9A.nota2t.splice(index, 1);
                nomes9A.nota3t.splice(index, 1);
                setLocalStorage(nomes9A);
                atualizarTabela();
                registrarNoDashboard('excluido', nome, '9º Ano - A', motivo);
...
function preparaEditacao(index) {
    let nomes9A = getLocalStorage();
    editaId = index;
    const nome1 = nomes9A.nomet[index];
    ...
    if (confirmar) {
        ...
        if (senhae === '12345') {
            ...
            nome.value = nome1;
            nota1.value = nomes9A.nota1t[index];
            nota2.value = nomes9A.nota2t[index];
            nota3.value = nomes9A.nota3t[index];
