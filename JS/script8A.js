...
const nomes8A = {
    nomet: [
        "AnaBeatrizSilva", "BrunoCostaAlmeida", "CarlaLimaSantos",
        "DiegoOliveiraCosta", "EduardaPereiraLima", "FelipeRodriguesAlmeida",
        "GabrielaCostaOliveira", "HugoLimaPereira", "IsabellySantosCosta",
        "JoãoPedroAlmeidaLima", "KarinaOliveiraSantos", "LeonardoCostaPereira",
        "MarinaAlmeidaOliveira", "NathanLimaCosta", "OliviaSantosAlmeida",
        "PedroHenriquePereiraLima", "RafaelaCostaOliveira", "SamuelAlmeidaSantos",
        "TaináPereiraCosta", "VictorHugoLimaOliveira", "YasminAlmeidaPereira",
...
function memorizaNomesEmPartes() { localStorage.setItem('nomes8A', JSON.stringify(nomes8A)); }
if (!localStorage.getItem('nomes8A')) { memorizaNomesEmPartes(); }

function getLocalStorage() { return JSON.parse(localStorage.getItem('nomes8A')) ?? { nomet: [], nota1t: [], nota2t: [], nota3t: [] }; }
function setLocalStorage(nomes8A) { localStorage.setItem('nomes8A', JSON.stringify(nomes8A)); }

function ordenarNomesComNotas(nomes8A) {
    const combinados = nomes8A.nomet.map((nome, index) => ({ nome: nome, nota1: nomes8A.nota1t[index], nota2: nomes8A.nota2t[index], nota3: nomes8A.nota3t[index] }));
    combinados.sort((a, b) => a.nome.localeCompare(b.nome));
    nomes8A.nomet = combinados.map(item => item.nome);
    nomes8A.nota1t = combinados.map(item => item.nota1);
    nomes8A.nota2t = combinados.map(item => item.nota2);
    nomes8A.nota3t = combinados.map(item => item.nota3);
}
...
function atualizarTabela() {
    ...
    const nomes8A = getLocalStorage();
    nomes8A.nomet.forEach((nome, index) => { inserirLinhaTabela(nome, index, nomes8A.nota1t[index], nomes8A.nota2t[index], nomes8A.nota3t[index]); });
}
...
function inserirNota() {
    ...
    if (parseFloat(aluno.nota1) <= 10 && parseFloat(aluno.nota2) <= 10 && parseFloat(aluno.nota3) <= 10) {
        if (parseFloat(aluno.nota1) >= 0 && parseFloat(aluno.nota2) >= 0 && parseFloat(aluno.nota3) >= 0) {
            let nomes8A = getLocalStorage();
            if (editaId === null) {
                nomes8A.nomet.push(aluno.nome);
                nomes8A.nota1t.push(aluno.nota1);
                nomes8A.nota2t.push(aluno.nota2);
                nomes8A.nota3t.push(aluno.nota3);
                ordenarNomesComNotas(nomes8A);
                setLocalStorage(nomes8A);
                registrarNoDashboard('adicionado', aluno.nome, '8º Ano - A', 'Novo aluno matriculado');
                alert("Aluno adicionado com sucesso!");
                atualizarTabela();
                add.style.display = 'block';
                formulario.style.display = 'none';
            } else { editarNota(nomes8A, editaId, aluno); }
        } else { alert('Coloque uma nota entre 0 e 10'); }
    } else { alert('Coloque uma nota entre 0 e 10'); }
}

function editarNota(nomes8A, index, aluno) {
    nomes8A.nomet[index] = aluno.nome;
    nomes8A.nota1t[index] = aluno.nota1;
    nomes8A.nota2t[index] = aluno.nota2;
    nomes8A.nota3t[index] = aluno.nota3;
    ordenarNomesComNotas(nomes8A);
    setLocalStorage(nomes8A);
    botao.value = "Inserir";
    editaId = null;
...
function excluirNota(index) {
    let nomes8A = getLocalStorage();
    const nome = nomes8A.nomet[index];
    ...
    if (motivo) {
        ...
        if (confirmarExclusao) {
            ...
            if (senha === '12345') {
                ...
                nomes8A.nomet.splice(index, 1);
                nomes8A.nota1t.splice(index, 1);
                nomes8A.nota2t.splice(index, 1);
                nomes8A.nota3t.splice(index, 1);
                setLocalStorage(nomes8A);
                atualizarTabela();
                registrarNoDashboard('excluido', nome, '8º Ano - A', motivo);
...
function preparaEditacao(index) {
    let nomes8A = getLocalStorage();
    editaId = index;
    const nome1 = nomes8A.nomet[index];
    ...
    if (confirmar) {
        ...
        if (senhae === '12345') {
            ...
            nome.value = nome1;
            nota1.value = nomes8A.nota1t[index];
            nota2.value = nomes8A.nota2t[index];
            nota3.value = nomes8A.nota3t[index];
